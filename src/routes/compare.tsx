...
import { X, GitCompareArrows, ShieldCheck } from "lucide-react";
...
function ComparePage() {
  ...
  const proceedToTestDrive = async (vehicleId: string) => {
    ...
    try {
      ...
      toast.error("Couldn't reach our security check. Please try again.");
    } finally {
      setVerifying(null);
    }
  };
  ...
  const fmtCO2 = (n: number | null) => n != null ? `${n} g/km` : "—";

  const specs: { label: string; get: (v: V) => string; group?: string }[] = [
    ...
    { label: "CO₂ emissions", get: (v) => fmtCO2(v.co2_emissions) },
    { label: "Road tax band", get: (v) => v.road_tax_band || "—" },
    { label: "MOT expiry", get: (v) => fmtDate(v.mot_expiry) },
      ...
      <section className="border-b border-border bg-muted/30 py-10">
        <div className="mx-auto flex max-w-7xl flex-wrap items-end justify-between gap-4 px-4 sm:px-6 lg:px-8">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">{ids.length} of 3 selected</p>
          </div>
        ...
        {loading ? (
          ...
          <>
                    ...
                    {rows.map((v) => (
                      <td key={v.id} className="border-t border-border p-3">
                        <Button
                          size="sm"
                          className="w-full"
                          disabled={v.sold || verifying === v.id}
                          onClick={() => proceedToTestDrive(v.id)}
                        >
                          {verifying === v.id ? "Verifying…" : v.sold ? "Sold" : "Book test drive"}
                        </Button>
                      </td>
                    ))}
                  </tr>
            ...
            <p className="mt-4 flex items-center gap-2 text-xs text-muted-foreground">
              ...
              Booking is protected by reCAPTCHA — we verify each request before continuing.
            </p>
          </>
        )}
      </section>
...
function CompareSkeleton({ count }: { count: number }) {
  ...
  return (
    <div className="overflow-x-auto" aria-busy="true" aria-label="Loading comparison">
      <table className="w-full min-w-[640px] border-collapse">
        <thead>
          <tr>
        ...
        <tbody>
          {Array.from({ length: 10 }).map((_, r) => (
            <tr key={r} className={r % 2 === 0 ? "bg-card" : ""}>
              <td className="border-t border-border p-3">
                <div className="h-3 w-20 animate-pulse rounded bg-muted" />
              </td>
              {Array.from({ length: cols }).map((_, c) => (
                <td key={c} className="border-t border-border p-3">
